...
public class Parser {
     ...
     *
     * <pre>
     *    interfaceBody  ::=  INTERFACE  IDENTIFIER
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        ArrayList<JMember> interfaceBlock = interfaceBody();

        return new JInterfaceDeclaration(line, mods, name, superInterfaces, interfaceBlock);
    }
    ...
    /**
     * Parses a interfaces body and returns a list of members in the body.
     *
     * <pre>
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            mustBe(SEMI);
            return new JDoStatement(line, statement, expr);
            ...
            mustBe(LPAREN);
            ArrayList<JStatement> initExpression = null;
            if (!see(SEMI)) {
                initExpression = forInit();
            }
            mustBe(SEMI);
            ...
            mustBe(SEMI);
            ArrayList<JStatement> updateExpression = null;
            if (!see(RPAREN)) { // for (;;) and for (int i = 0, x = 0, y = 0, double e =3; i < 12;
                // i++)
                updateExpression = forUpdate();
            }
            mustBe(RPAREN);
            JStatement statement = statement();
            return new JForStatement(line, initExpression, expr, updateExpression, statement);
        } else if (have(BREAK)) {
            mustBe(SEMI);
            return new JBreakStatement(line);
            ...
            JExpression expr = parExpression();
            mustBe(LCURLY);
            ArrayList<SwitchStatementGroup> smtgroup = new ArrayList<>();
            while(!see(RCURLY) && !see(EOF)) {
                smtgroup.add(switchBlockStatementGroup());
            }
            mustBe(RCURLY);
            return new JSwitchStatement(line, expr, smtgroup);
        } else if (have(TRY)) {
            ArrayList<JBlock> blocks = new ArrayList<>();
            ArrayList<JFormalParameter> paremeters = new ArrayList<>();
            ...
            while (see(CATCH)) {
                mustBe(CATCH);
                mustBe(LPAREN);
                paremeters.add(formalParameter());
                mustBe(RPAREN);
                blocks.add(block());
            }
            ...
            return new JTryStatement(line, tryblock, paremeters, blocks, finallyblock);
        } else if (have(THROW)) {
            JExpression expr = expression();
            mustBe(SEMI);
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ArrayList<JExpression> switchLabel = new ArrayList<>();
        ArrayList<JStatement> block = new ArrayList<>();


        if (see(CASE)) {
            mustBe(CASE);
            switchLabel.add(expression());
        } else {
            mustBe(DEFAULT);
            switchLabel.add(null);
        }
        mustBe(COLON);
        ...
        return new SwitchStatementGroup(switchLabel,block);
    ...
    private ArrayList<JStatement> forInit() {
        ...
        if (seeBasicType()) {
            ArrayList<JVariableDeclarator> list = variableDeclarators(type());
            statements.add(new JVariableDeclaration(line,list));
        } else {
    ...
    private JExpression conditionalExpression() {
        ...
        if (have(QUESTION)) {
            JExpression branch = expression();
            mustBe(COLON);
            lhs = new JConditionalExpression(line, lhs, branch, conditionalAndExpression());
        }
        return lhs;
    }
